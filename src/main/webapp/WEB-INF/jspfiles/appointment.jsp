...
<div class="card-container">
    <div class="info-card">
        ...
        <div class="card mt-4">
            <div class="card-body">
                <h5 class="card-title">${appointment.name} - ${appointment.type}</h5>

                <c:if test="${appointment.car ne null}">
                    <p class="card-text">Car: ${appointment.car.carName}</p>
                    <p class="card-text">Brand: ${appointment.car.carBrand}</p>
                    <p class="card-text">RC: ${appointment.car.registrationNumber}</p>
                    <p class="card-text">Fuel: ${appointment.car.fuelType}</p>
                    <p class="card-text">Price: ₹${appointment.car.price}</p>
                </c:if>

                <p class="card-text">Date: ${appointment.date}</p>
                <div class="text-center">
                    <a href="/updateappointment/${appointment.appointmentId}" class="btn btn-primary mr-2">Update</a>
                    <a href="/deleteappointment/${appointment.appointmentId}" class="btn btn-danger">Delete</a>
                </div>
            </div>
        </div>
    </div>
